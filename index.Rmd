...
Scenario:
- Fast approximate model $p(\theta~\vert~y)$ known to be biased
...
3. Sample posterior $(\theta_i \vert \tilde{y}_m) \sim p(~\cdot~\vert~\tilde{y}_m)$
<br>
...
**Bias correction**
<br>
Using pairs $(\theta_i, \{\theta_j\}_{i=1}^{N})$ estimate average bias

$$\text{bias} = \mathbb{E}[\mathbb{E}(\theta \vert \tilde{y}_m) - \bar\theta_m]$$
]
...
`Approx posterior` = paste0("p(\u03B8 | y[",1:9,"])"))
...
- No true distribution to compare to!
