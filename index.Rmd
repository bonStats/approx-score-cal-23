...
where $\hat{\mu}(y) = \mathbb{E}(\theta),\quad \theta \sim \hat\Pi(\cdot~\vert~y)$ for $y \in \mathsf{Y}$.
...
--
- Optimal transformation and calibration checks are *concentrated* on $\bar\Pi$ because of finite sample approximation

--
- Even more so when manipulating the weights of the method
...
--
- Use flexibility of $v(\tilde{y})$ to obtain $w(\theta, \tilde{y}) \approx 1$ apply to control weights in a SNP-style or "...GNN via Scoring Rule Minimization" (Pacchiardi & Dutta 2022)
