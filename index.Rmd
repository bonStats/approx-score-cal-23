...
3. Sample posterior $\theta_{m,i} \sim \hat{\Pi}(~\cdot~\vert~\tilde{y}_m)$
]]
...
1. Estimate average bias $$\text{bias} = \sum_{m=1}^{M}\sum_{i=1}^{N}\left(\theta_{m,i} - \bar\theta_m \right)$$
2. Correction $\theta \sim \hat{\Pi}(~\cdot~\vert~y)$  $$\theta^\prime = \theta - \text{bias}$$
]
]
